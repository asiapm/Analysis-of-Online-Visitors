...
head(data)
filter(data, data$SpecialDay==0)
ggplot(data = data, aes(x=data$VisitorType, y=data$ProductRelated))+
  geom_point()

ggplot(data = data, aes(x=data$VisitorType, y=data$ProductRelated, colour=data$VisitorType))+
  geom_point()


#y axis numbers too high

ggplot(data, aes(x=data$VisitorType, y=data$ProductRelated) ) + 
  geom_bar(stat="identity", fill="steelblue")
scale_y_continuous(labels=scales::comma)

ggplot(data, aes(x=data$Revenue, y=v) ) + 
  geom_bar(stat="identity", fill="steelblue")+
  scale_y_continuous(labels=scales::comma)+
